...
class Trader:
    ...
    def __init__(self):
        ...
        self.residual_threshold = 0.001  # 3% IV deviation from curve
    ...
    def basket_1_strategy(self, state: TradingState):
        ...
        order_depth_pb1 = state.order_depths[PICNIC_BASKET1]
        ...
        order_depth_cr = state.order_depths[CROISSANTS]
        ...
        order_depth_dje = state.order_depths[DJEMBES]
    ...
    def basket_2_strategy(self, state: TradingState):
        ...
        order_depth_pb2 = state.order_depths[PICNIC_BASKET2]
    ...
    def VOLCANIC_strat(self, state: TradingState):
        ...
        for voucher in self.vouchers:
            ...
            tte = state.timestamp / (1000000 * 365) + 2/365
        ...
        for voucher in self.vouchers:
            ...
            delta = self.adjusted_delta_hedge(coeffs, delta, base_iv, tte)
    ...
    def adjusted_delta_hedge(self, coeffs, original_delta, base_iv, tte):
        ...
        convexity_adj = 1 + tte * coeffs[0]  # Example adjustment
    ...
    def macarons_adj_price(self, state: TradingState):


        macaron_obs = state.observations.conversionObservations[MAGNIFICENT_MACARONS]
        macaron_bid = macaron_obs.bidPrice
        macaron_ask = macaron_obs.askPrice
        import_fees = macaron_obs.importTariff + macaron_obs.transportFees
        macaron_import = macaron_ask + import_fees
        export_fees = macaron_obs.exportTariff + macaron_obs.transportFees + 0.1
        macaron_export = macaron_bid - export_fees
        return macaron_import, macaron_export
    
    def fair_price_lin(self, data, time):
        coeffs = np.polyfit(data, time, 2)
        result = (lambda x: coeffs[0]*x**2 + coeffs[1]*x + coeffs[2])(time[-1])
        return result
    
    def macaron_strat(self, state: TradingState):
        edge = 2
        curr_time = state.timestamp
        vals = self.macaron_vals
        orders: List[Order] = []
        position = state.position[MAGNIFICENT_MACARONS] if MAGNIFICENT_MACARONS in state.position else 0
        macaron_obs = state.observations.conversionObservations[MAGNIFICENT_MACARONS]
        best_ask = min(state.order_depths[MAGNIFICENT_MACARONS].sell_orders.keys())
        best_bid = max(state.order_depths[MAGNIFICENT_MACARONS].buy_orders.keys())
        mid_price = (best_ask + best_bid) / 2

        logger.print(best_ask,state.order_depths[MAGNIFICENT_MACARONS].sell_orders[best_ask], 
                     best_bid, state.order_depths[MAGNIFICENT_MACARONS].buy_orders[best_bid])

        if len(vals['time']) >= vals['lookback_long']:
            vals['time'].pop(0)

        if len(vals['long_hist']) >= vals['lookback_long']:
            vals['long_hist'].pop(0)

        if len(vals['long_hist_no']) >= vals['lookback_long_no']:
            vals['long_hist_no'].pop(0)

        if len(vals['short_hist']) >= vals['lookback_short']:
            vals['short_hist'].pop(0)
        
        if len(vals['short_hist_no']) >= vals['shorter_lookback']:
            vals['short_hist_no'].pop(0)
        vals['long_hist'].append(mid_price)
        vals['long_hist_no'].append(mid_price)
        vals['short_hist'].append(mid_price)
        vals['short_hist_no'].append(mid_price)
        vals['time'].append(curr_time)
        
        if macaron_obs.sunlightIndex < self.macaron_vals['csi']:
            vals['manual'] = True
            logger.print(np.mean(vals['short_hist']), mid_price)
            if np.mean(vals['short_hist']) > np.mean(vals['long_hist']) + edge:
                
                quantity = min(self.position_limit[MAGNIFICENT_MACARONS] - position, 
                    abs(state.order_depths[MAGNIFICENT_MACARONS].sell_orders[best_ask]))
                orders.append(Order(MAGNIFICENT_MACARONS, best_ask, quantity))
                logger.print('going long', best_ask, quantity)
            if np.mean(vals['short_hist']) < np.mean(vals['long_hist']) - edge:
                vals['short'] = True
                quantity = min(self.position_limit[MAGNIFICENT_MACARONS] + position, 
                    abs(state.order_depths[MAGNIFICENT_MACARONS].buy_orders[best_bid]))
                orders.append(Order(MAGNIFICENT_MACARONS, best_bid, -quantity))
                logger.print('going short', best_bid, -quantity)


        
        if macaron_obs.sunlightIndex > self.macaron_vals['csi'] and vals['manual'] == True:
            if position == 0:
                vals['manual'] = False
                vals['short'] = False
            vals['max_track'] = []
            if position <0:
                quantity = min(position, 
                    state.order_depths[MAGNIFICENT_MACARONS].sell_orders[best_ask])
                orders.append(Order(MAGNIFICENT_MACARONS, best_ask, -quantity))
            if position >0:
                quantity = min(position, 
                    abs(state.order_depths[MAGNIFICENT_MACARONS].buy_orders[best_bid]))
    ...
    def run(self, state: TradingState):
        ...
        result[MAGNIFICENT_MACARONS] = self.macaron_strat(state)
