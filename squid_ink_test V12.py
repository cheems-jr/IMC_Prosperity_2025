    ...
    def compress_order_depths(self, order_depths: dict[Symbol, OrderDepth]) -> dict[Symbol, list[Any]]:
        ...
        if order_depths:
             for symbol, order_depth in order_depths.items():
                 buy_orders_data = getattr(order_depth, 'buy_orders', {}); sell_orders_data = getattr(order_depth, 'sell_orders', {})
    ...
    def compress_trades(self, trades: dict[Symbol, list[Trade]]) -> list[list[Any]]:
        ...
        if trades:
            for arr in trades.values():
                if arr:
                     for trade in arr:
                         buyer=getattr(trade,'buyer',None);seller=getattr(trade,'seller',None);timestamp=getattr(trade,'timestamp',0)
                         compressed.append([trade.symbol,trade.price,trade.quantity,buyer,seller,timestamp])
        return compressed
        ...
        if observations:
            ...
            if conv_obs_data:
                 for product, observation in conv_obs_data.items():
                     conv_obs[product]=[getattr(observation, attr, None) for attr in ['bidPrice','askPrice','transportFees','exportTariff','importTariff','sugarPrice','sunlightIndex']]
    ...
    def compress_orders(self, orders: dict[Symbol, List[Order]]) -> list[list[Any]]:
        ...
        if orders:
            for arr in orders.values():
                 if arr:
                     for order in arr: compressed.append([order.symbol, int(order.price), order.quantity])
        return compressed
...
class Trader:
    """
    Market Making Strategy V12: Adaptive quotes based on EMA slope.
    """
    def __init__(self):
