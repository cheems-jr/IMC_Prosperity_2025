...
class Trader:
    ...
    def VOLCANIC_strat(self, state: TradingState):
        ...
        for voucher in self.vouchers:
            ...
            tte = state.timestamp / (1000000 * 365) + 3/365
    ...
    def macaron_strat(self, state: TradingState):
        ...
        time = np.arange(curr_time - 9900, curr_time, 10)
        ...
        vals['max_price'] = max(vals['long_hist'])
        ...
        if macaron_obs.sunlightIndex <= self.macaron_vals['csi']:
            ...
            if mid_price * 0.99 <= vals['max_price'] or vals['short'] == True:
                vals['short'] = True
                quantity = min(self.position_limit[MAGNIFICENT_MACARONS] + position, 
        ...
        if macaron_obs.sunlightIndex > self.macaron_vals['csi'] and vals['short'] == True:
            vals['short'] == False
        ...
        '''else:
            fair_price_long = self.fair_price_lin(vals['long_hist'], time)
            fair_price_short = self.fair_price_lin(vals['short_hist'], time[-5:])

            if fair_price_long > fair_price_short:
                orders.append(MAGNIFICENT_MACARONS, best_bid, quantity)
            elif fair_price_long < fair_price_short:                
                orders.append(MAGNIFICENT_MACARONS, best_ask, quantity)'''
